<template>
  <div class="resource-type-management-container">
    ...
    <el-card shadow="hover" class="table-card">
      ...
      <div class="action-buttons">
        <el-button
          v-if="userStore.hasPermission('resource_type:create')"
          type="success"
          :icon="Plus"
          @click="handleCreate"
          class="create-button"
      ...
      >
        ...
        <el-table-column label="操作" width="180" align="center" fixed="right">
          <template #default="scope">
            <el-button
              v-if="userStore.hasPermission('resource_type:update')"
              type="primary"
              size="small"
              :icon="Edit"
            ...
            <el-popconfirm
              v-if="
                userStore.hasPermission('resource_type:delete') &&
                !scope.row.is_system
              "
              title="确定要删除该资源类型吗? 这可能会影响关联的权限。"
    ...
    <ResourceTypeFormDialog
      ...
      :resource-type-id="selectedResourceTypeId"
...
import { getResourceTypes, deleteResourceType } from "@/api/permission";
import type { ResourceType } from "@/types/api";
import ResourceTypeFormDialog from "./components/ResourceTypeFormDialog.vue";
...
const selectedResourceTypeId = ref<number | null>(null);
// --- 对话框状态 End ---
...
const fetchResourceTypes = async () => {
  ...
  try {
    // 获取全部资源类型列表
    let allTypes = await getResourceTypes();
    // 前端筛选逻辑
    if (listQuery.code) {
...
// 处理新增按钮点击
const handleCreate = () => {
  selectedResourceTypeId.value = null;
  dialogVisible.value = true;
};

// 处理编辑按钮点击
const handleEdit = (row: ResourceType) => {
  selectedResourceTypeId.value = row.id;
  dialogVisible.value = true;
};

// 处理删除按钮点击
const handleDelete = async (resourceTypeId: number) => {
  deleteLoading[resourceTypeId] = true;
  try {
    await deleteResourceType(resourceTypeId);
    ElMessage.success("资源类型删除成功！");
    fetchResourceTypes(); // 重新加载列表
  } catch (error) {
    console.error("删除资源类型失败:", error);
    // 错误消息由拦截器处理
  } finally {
    delete deleteLoading[resourceTypeId];
  }
};
...
.resource-type-management-container {
  padding: 20px;
}
